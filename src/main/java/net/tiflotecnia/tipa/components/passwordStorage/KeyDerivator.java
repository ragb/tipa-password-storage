...
/**
 * Class to derive keys from passwords, using various algorithms; keys are
 * suitable for password storage.
 * <p>
 * This class uses typesafe configuration for it's parameterization. All keys
 * are kept on the net.tiflotecnia.components.passwordStorage config subtree.
 * For the used parameters and format please check the reference configuration
 * file in src/main/resources/reference.conf
 * <p>
 * With the algorithms subkey, we can add more algorithms, even external from
 * this package. See the bundled sscrypt and plain examples.
 * 
 * @author Rui Batista
 * @see Config
 * @see ConfigFactory
...
public class KeyDerivator {
	 ...
	 *            the password
	 * @param algorithm
	 *            the algorithm to use (algorithm identifier)
	...
	private void populateAlgorithms() {
		for (Config c : _config.getConfigList("algorithms")) {
			...
			try {
				...
				algorithm = (AbstractKeyDerivationAlgorithm) classRef
						.getConstructor(Config.class).newInstance(params);
				...
				throw new RuntimeException(
						"Error creating key derivation algorithm class", e);
